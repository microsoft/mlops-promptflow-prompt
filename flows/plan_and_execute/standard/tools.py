    ...
    for i in range(number_of_results):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                # Limit the length of the content to 2000 characters
                text = text[:5000]
            else:
...
def wikipedia_tool(query: Annotated[str, tool_descriptions["wikipedia_tool"]["query"]],
    ...
    for title in results:
        try:
            # Attempt to get the page for each result
...
def math_tool(problem_description: Annotated[str, tool_descriptions["math_tool"]["problem_description"]],
    ...
    math_executor = UserProxyAgent(
        name="TOOL_EXECUTOR",
        description="""
        An agent that acts as a proxy for the user and executes the suggested function calls from MATH_ASSISTANT.
        """,
        code_execution_config=False,
        is_termination_msg=is_termination_msg,
