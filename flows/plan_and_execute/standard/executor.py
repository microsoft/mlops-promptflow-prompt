...
def worker_tool(connection: CustomConnection, plan: str) -> str:
    """THis node is responsible for executing the plan generated by the planner node."""
    ...
    def execute_plan_parallel(plan, agent):
        def has_unresolved_dependencies(item, resolved_ids):
            ...
            for arg in args.values():
                ...
                any(ref_id for ref_id in plan_ids if ref_id not in resolved_ids and ref_id in arg):
                    return True
            return False
        ...
        def substitute_dependency(id, original_argument_value, dependency_value):
            instruction = """
            Extract the entity name or fact from the dependency value in a way that makes sense to use it to substitute 
            the variable #E in the original argument value.
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            ...
            for item_id, item in plan_ids.items():
                if not has_unresolved_dependencies(item, resolved_ids):
                    ...
                    future = executor.submit(agent.execute_function,
